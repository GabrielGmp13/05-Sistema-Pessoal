...
import ImportarNubank from '@/components/financas/ImportarNubank'
import AnalisesGastos from '@/components/financas/AnalisesGastos'
...
export default function FinancasPage() {
  ...
  const resumoCarteira = useMemo(() => investimentos.reduce((resumo, item) => {
    const custo = Number(item.quantidade) * Number(item.preco_medio)
    const cotacao = cotacoes[item.ticker]
    resumo.custo += custo
    if (cotacao) {
      resumo.valorAtual += Number(item.quantidade) * cotacao.preco
      resumo.custoCotado += custo
      resumo.posicoesCotadas += 1
    }
    return resumo
  }, { custo: 0, custoCotado: 0, valorAtual: 0, posicoesCotadas: 0 }), [cotacoes, investimentos])
  ...
  return (
    <main className="min-h-[calc(100vh-3.5rem)] bg-background text-foreground"><div className="mx-auto w-full max-w-6xl px-4 py-8 sm:px-6">
      ...
      <ImportarNubank categorias={categorias} onImportado={carregar} />
      <AnalisesGastos periodo={periodo} lancamentos={lancamentos} categorias={categorias} />
      ...
      <section className="mt-10 border-t border-border pt-5">
        ...
        {resumoCarteira.posicoesCotadas > 0 ? <div className="mt-4 grid gap-3 sm:grid-cols-3"><Resumo icon={WalletCards} label={`Valor atual (${resumoCarteira.posicoesCotadas}/${investimentos.length} cotadas)`} valor={resumoCarteira.valorAtual} positive /><Resumo icon={TrendingUp} label="Resultado das posições cotadas" valor={resumoCarteira.valorAtual - resumoCarteira.custoCotado} positive={resumoCarteira.valorAtual >= resumoCarteira.custoCotado} /><div className="rounded-lg border border-border bg-card p-4"><p className="text-xs text-muted-foreground">Cobertura de cotação</p><Progress className="mt-3" value={(resumoCarteira.posicoesCotadas / Math.max(investimentos.length, 1)) * 100} /><p className="mt-2 text-xs text-muted-foreground">Valores parciais até consultar todas as posições.</p></div></div> : null}
