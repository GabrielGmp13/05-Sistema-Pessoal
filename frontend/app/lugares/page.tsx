...
export default function LugaresPage() {
  ...
  function limpar() { setEditando(null); setForm(FORM_VAZIO) }
  ...
  async function salvar() {
    ...
    setSalvando(true)
    setErro(null)
    const resultado = await salvarLugar({
      ...
      texto: form.texto.trim() || null, capa_url: form.capa_url.trim() || null,
    }, editando?.uuid)
    if (!resultado) setErro('Não foi possível salvar. Revise datas, coordenadas e valores.')
    else { limpar(); await carregar(); setSelecionado(resultado) }
    setSalvando(false)
  }

  async function confirmarExclusao() {
    if (!excluir) return
    await deletarLugar(excluir.uuid)
