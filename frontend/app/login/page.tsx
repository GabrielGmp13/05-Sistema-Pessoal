...
export default function LoginPage() {
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    setCarregando(true)
    if (mode === 'signup') {
      const { data, error } = await sb.auth.signUp({ email, password: senha, options: { emailRedirectTo: `${window.location.origin}/auth/confirm` } })
      setCarregando(false)
      if (error) { setErro('Não foi possível criar a conta. Confira os dados ou tente mais tarde.'); return }
  ...
  return <div className="login-container"><form onSubmit={handleSubmit} className="login-form">
    ...
    <button type="submit" disabled={carregando} className="btn-salvar">{carregando ? 'Aguarde...' : mode === 'signup' ? 'Criar conta' : 'Entrar'}</button>
    ...
    {signupEnabled ? <button type="button" className="login-secondary" onClick={() => { setMode(mode === 'login' ? 'signup' : 'login'); setErro(null); setMensagem(null) }}>{mode === 'login' ? 'Ainda não tenho conta' : 'Já tenho uma conta'}</button> : <p className="login-note">Novas contas ainda são liberadas por convite.</p>}
  </form></div>
}
