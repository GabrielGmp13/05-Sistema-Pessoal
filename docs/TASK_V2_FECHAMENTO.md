...
atual, com recusa para segunda conta e acesso bruto anônimo. A migration de
...
| I09 Segurança/dados/publicação | `00200`/`00300` aplicadas; testes locais com duas contas cobriram RLS/alteração cruzada, FK, exportação das novas tabelas e foto histórica privada. Lote publicado com stage sem segredos/PDF, 147 testes, typecheck, lint, build, CI e deploy aprovados. O PDF avulso continua fora do Git. `00100` segue sem aplicação. | Completar matriz de sessão/cookies, temas/rotas e fluxos reais no candidato publicado, inclusive aparelho físico. Revalidar após correções; não interpretar smoke sem sessão como homologação integral. |
